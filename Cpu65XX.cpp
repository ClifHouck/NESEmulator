...
buildInstructionSet() 
{
    ...
    m_instructions[0xB5] = Instruction(0xB5, 2, "LDA", cycleFunc, workFunc);
    
    // AD nn nn  nz----  4   LDA nnnn    Load A with Absolute      A=[nnnn]
    workFunc    = [this] { setA(m_memory.byteAt(wordOperand())); }; 
    m_instructions[0xAD] = Instruction(0xAD, 3, "LDA", cycleFunc, workFunc);
    ...
    m_instructions[0xBD] = Instruction(0xBD, 3, "LDA", cycleFunc, workFunc);
    // B9 nn nn  nz----  4*  LDA nnnn,Y  Load A with Absolute,Y    A=[nnnn+Y]
    workFunc    = [this] { setA(m_memory.byteAt(wordOperand() + Y())); }; 
    m_instructions[0xB9] = Instruction(0xB9, 3, "LDA", cycleFunc, workFunc);
    ...
    m_instructions[0xA1] = Instruction(0xA1, 2, "LDA", cycleFunc, workFunc);
    ...
    m_instructions[0x7D] = Instruction(0x7D, 3, "ADC", cycleFunc, workFunc);
    ...
    m_instructions[0x79] = Instruction(0x79, 3, "ADC", cycleFunc, workFunc);
    ...
    m_instructions[0x61] = Instruction(0x61, 2, "ADC", cycleFunc, workFunc);
    ...
    m_instructions[0x71] = Instruction(0x71, 2, "ADC", cycleFunc, workFunc);
    ...
    m_instructions[0xE9] = Instruction(0xE9, 2, "SBC", cycleFunc, workFunc);
    // E5 nn     nzc--v  3   SBC nn      Subtract Zero Page      A=A+C-1-[nn]
    cycleFunc   = [this] { return 3; }; 
    workFunc    = [this] { setA(subtractionWithBorrow(A(), m_memory.byteAt(byteOperand()))); };
    m_instructions[0xE5] = Instruction(0xE5, 2, "SBC", cycleFunc, workFunc);
    ...
    m_instructions[0xF5] = Instruction(0xF5, 2, "SBC", cycleFunc, workFunc);
    ...
    m_instructions[0xED] = Instruction(0xED, 3, "SBC", cycleFunc, workFunc);
    ...
    m_instructions[0xFD] = Instruction(0xFD, 3, "SBC", cycleFunc, workFunc);
    ...
    m_instructions[0xF9] = Instruction(0xF9, 3, "SBC", cycleFunc, workFunc);
    ...
    m_instructions[0xE1] = Instruction(0xE1, 2, "SBC", cycleFunc, workFunc);
    ...
    m_instructions[0xF1] = Instruction(0xF1, 2, "SBC", cycleFunc, workFunc);
    ...
    cycleFunc   = [this] { return 2; }; 
    workFunc    = [this] { setA(A() ^ byteOperand()); };
    m_instructions[0x49] = Instruction(0x49, 2, "EOR", cycleFunc, workFunc);
    // 45 nn     nz----  3   EOR nn      XOR Zero Page      A=A XOR [nn]
    cycleFunc   = [this] { return 3; }; 
    workFunc    = [this] { setA(A() ^ m_memory.byteAt(byteOperand())); };
    m_instructions[0x45] = Instruction(0x45, 2, "EOR", cycleFunc, workFunc);
    // 55 nn     nz----  4   EOR nn,X    XOR Zero Page,X    A=A XOR [nn+X]
    cycleFunc   = [this] { return 4; }; 
    workFunc    = [this] { setA(A() ^ m_memory.byteAt(byteOperand() + X())); };
    m_instructions[0x55] = Instruction(0x55, 2, "EOR", cycleFunc, workFunc);
    // 4D nn nn  nz----  4   EOR nnnn    XOR Absolute       A=A XOR [nnnn]
    workFunc    = [this] { setA(A() ^ m_memory.byteAt(wordOperand())); };
    m_instructions[0x4D] = Instruction(0x4D, 3, "EOR", cycleFunc, workFunc);
    // 5D nn nn  nz----  4*  EOR nnnn,X  XOR Absolute,X     A=A XOR [nnnn+X]
    cycleFunc   = [this] { return 4 + crossesPageBoundary(wordOperand() + X()); }; 
    workFunc    = [this] { setA(A() ^ m_memory.byteAt(wordOperand() + X())); };
    m_instructions[0x5D] = Instruction(0x5D, 3, "EOR", cycleFunc, workFunc);
    // 59 nn nn  nz----  4*  EOR nnnn,Y  XOR Absolute,Y     A=A XOR [nnnn+Y]
    cycleFunc   = [this] { return 4 + crossesPageBoundary(wordOperand() + Y()); }; 
    workFunc    = [this] { setA(A() ^ m_memory.byteAt(wordOperand() + Y())); };
    m_instructions[0x59] = Instruction(0x59, 3, "EOR", cycleFunc, workFunc);
    // 41 nn     nz----  6   EOR (nn,X)  XOR (Indirect,X)   A=A XOR [[nn+X]]
    cycleFunc   = [this] { return 6; }; 
    workFunc    = [this] { setA(A() ^ m_memory.byteAt(m_memory.wordAt(byteOperand() + X()))); };
    m_instructions[0x41] = Instruction(0x41, 2, "EOR", cycleFunc, workFunc);
    // 51 nn     nz----  5*  EOR (nn),Y  XOR (Indirect),Y   A=A XOR [[nn]+Y]
    cycleFunc   = [this] { return 5 + crossesPageBoundary(m_memory.wordAt(byteOperand()) + Y()); }; 
    workFunc    = [this] { setA(A() ^ m_memory.byteAt(m_memory.wordAt(byteOperand()) + Y())); };
    m_instructions[0x51] = Instruction(0x51, 2, "EOR", cycleFunc, workFunc);
    ...
    cycleFunc   = [this] { return 2; }; 
    workFunc    = [this] { setA(A() | byteOperand()); };
    m_instructions[0x09] = Instruction(0x09, 2, "OR", cycleFunc, workFunc);
    // 05 nn     nz----  3   ORA nn      OR Zero Page       A=A OR [nn]
    cycleFunc   = [this] { return 3; }; 
    workFunc    = [this] { setA(A() | m_memory.byteAt(byteOperand())); };
    m_instructions[0x05] = Instruction(0x05, 2, "OR", cycleFunc, workFunc);
    // 15 nn     nz----  4   ORA nn,X    OR Zero Page,X     A=A OR [nn+X]
    cycleFunc   = [this] { return 4; }; 
    workFunc    = [this] { setA(A() | m_memory.byteAt(byteOperand() + X())); };
    m_instructions[0x15] = Instruction(0x15, 2, "OR", cycleFunc, workFunc);
    // 0D nn nn  nz----  4   ORA nnnn    OR Absolute        A=A OR [nnnn]
    workFunc    = [this] { setA(A() | m_memory.byteAt(wordOperand())); };
    m_instructions[0x0D] = Instruction(0x0D, 3, "OR", cycleFunc, workFunc);
    // 1D nn nn  nz----  4*  ORA nnnn,X  OR Absolute,X      A=A OR [nnnn+X]
    cycleFunc   = [this] { return 4 + crossesPageBoundary(wordOperand() + X()); }; 
    workFunc    = [this] { setA(A() | m_memory.byteAt(wordOperand() + X())); };
    m_instructions[0x1D] = Instruction(0x1D, 3, "OR", cycleFunc, workFunc);
    // 19 nn nn  nz----  4*  ORA nnnn,Y  OR Absolute,Y      A=A OR [nnnn+Y]
    cycleFunc   = [this] { return 4 + crossesPageBoundary(wordOperand() + Y()); }; 
    workFunc    = [this] { setA(A() | m_memory.byteAt(wordOperand() + Y())); };
    m_instructions[0x19] = Instruction(0x19, 3, "OR", cycleFunc, workFunc);
    // 01 nn     nz----  6   ORA (nn,X)  OR (Indirect,X)    A=A OR [[nn+X]]
    cycleFunc   = [this] { return 6; }; 
    workFunc    = [this] { setA(A() | m_memory.byteAt(m_memory.wordAt(byteOperand() + X()))); };
    m_instructions[0x01] = Instruction(0x01, 2, "OR", cycleFunc, workFunc);
    // 11 nn     nz----  5*  ORA (nn),Y  OR (Indirect),Y    A=A OR [[nn]+Y]
    cycleFunc   = [this] { return 5 + crossesPageBoundary(m_memory.wordAt(byteOperand()) + Y()); }; 
    workFunc    = [this] { setA(A() | m_memory.byteAt(m_memory.wordAt(byteOperand()) + Y())); };
    m_instructions[0x11] = Instruction(0x11, 2, "OR", cycleFunc, workFunc);
    ...
    m_instructions[0xC1] = Instruction(0xC1, 2, "CMP", cycleFunc, workFunc);
    ...
    m_instructions[0xE6] = Instruction(0xE6, 2, "INC", cycleFunc, workFunc);
    ...
    m_instructions[0xF6] = Instruction(0xF6, 2, "INC", cycleFunc, workFunc);
    ...
    m_instructions[0xEE] = Instruction(0xEE, 3, "INC", cycleFunc, workFunc);
    ...
    m_instructions[0xFE] = Instruction(0xFE, 3, "INC", cycleFunc, workFunc);
    // E8        nz----  2   INX         Increment X            X=X+1
    cycleFunc   = [this] { return 2; }; 
    workFunc    = [this] { setX(X() + 1); };
    m_instructions[0xE8] = Instruction(0xE8, 1, "INX", cycleFunc, workFunc);
    // C8        nz----  2   INY         Increment Y            Y=Y+1
    workFunc    = [this] { setY(Y() + 1); };
    m_instructions[0xC8] = Instruction(0xC8, 1, "INY", cycleFunc, workFunc);

    // Decrement by one
    // C6 nn     nz----  5   DEC nn      Decrement Zero Page    [nn]=[nn]-1
    cycleFunc   = [this] { return 5; }; 
    workFunc    = [this] { setByte(m_memory.byteAt(byteOperand()), m_memory.byteAt(byteOperand()) - 1); };
    m_instructions[0xC6] = Instruction(0xC6, 2, "DEC", cycleFunc, workFunc);
    ...
    m_instructions[0xD6] = Instruction(0xD6, 2, "DEC", cycleFunc, workFunc);
    // CE nn nn  nz----  6   DEC nnnn    Decrement Absolute     [nnnn]=[nnnn]-1
    workFunc    = [this] { setByte(m_memory.byteAt(wordOperand()), m_memory.byteAt(wordOperand()) - 1); };
    m_instructions[0xCE] = Instruction(0xCE, 3, "DEC", cycleFunc, workFunc);
    // DE nn nn  nz----  7   DEC nnnn,X  Decrement Absolute,X   [nnnn+X]=[nnnn+X]-1
    cycleFunc   = [this] { return 7; }; 
    workFunc    = [this] { setByte(m_memory.byteAt(wordOperand() + X()), m_memory.byteAt(wordOperand() + X()) - 1); };
    m_instructions[0xDE] = Instruction(0xDE, 3, "DEC", cycleFunc, workFunc);
    // CA        nz----  2   DEX         Decrement X            X=X-1
    cycleFunc   = [this] { return 2; }; 
    workFunc    = [this] { setX(X() - 1); };
    m_instructions[0xCA] = Instruction(0xCA, 1, "DEX", cycleFunc, workFunc);
    // 88        nz----  2   DEY         Decrement Y            Y=Y-1
    workFunc    = [this] { setY(Y() - 1); };
    m_instructions[0x88] = Instruction(0x88, 1, "DEY", cycleFunc, workFunc);

    //Shift Left
    // 0A        nzc---  2   ASL A       Shift Left Accumulator   SHL A
    cycleFunc   = [this] { return 2; }; 
    workFunc    = [this] { setA(shiftLeft(A())); };
    m_instructions[0x0A] = Instruction(0x0A, 1, "ASL", cycleFunc, workFunc);
    ...
    m_instructions[0x06] = Instruction(0x06, 2, "ASL", cycleFunc, workFunc);
    ...
    m_instructions[0x16] = Instruction(0x16, 2, "ASL", cycleFunc, workFunc);
    ...
    m_instructions[0x0E] = Instruction(0x0E, 3, "ASL", cycleFunc, workFunc);
    ...
    m_instructions[0x56] = Instruction(0x56, 2, "LSR", cycleFunc, workFunc);
    ...
    m_instructions[0x4E] = Instruction(0x4E, 3, "LSR", cycleFunc, workFunc);
    ...
    m_instructions[0x5E] = Instruction(0x5E, 3, "LSR", cycleFunc, workFunc);

    // Rotate Left through Carry
    // 2A        nzc---  2   ROL A       Rotate Left Accumulator  RCL A
    cycleFunc   = [this] { return 2; }; 
    workFunc    = [this] { setA(rotateLeftThroughCarry(A())); };
    m_instructions[0x2A] = Instruction(0x2A, 1, "ROL", cycleFunc, workFunc);
    ...
    m_instructions[0x26] = Instruction(0x26, 2, "ROL", cycleFunc, workFunc);
    ...
    m_instructions[0x36] = Instruction(0x36, 2, "ROL", cycleFunc, workFunc);
    ...
    m_instructions[0x2E] = Instruction(0x2E, 3, "ROL", cycleFunc, workFunc);
    ...
    m_instructions[0x3E] = Instruction(0x3E, 3, "ROL", cycleFunc, workFunc);

    // Rotate Right through Carry
    // 6A        nzc---  2   ROR A       Rotate Right Accumulator RCR A
    cycleFunc   = [this] { return 2; }; 
    workFunc    = [this] { setA(rotateRightThroughCarry(A())); };
    m_instructions[0x6A] = Instruction(0x6A, 1, "ROR", cycleFunc, workFunc);
    ...
    m_instructions[0x66] = Instruction(0x66, 2, "ROR", cycleFunc, workFunc);
    ...
    m_instructions[0x76] = Instruction(0x76, 2, "ROR", cycleFunc, workFunc);
    ...
    m_instructions[0x6E] = Instruction(0x6E, 3, "ROR", cycleFunc, workFunc);
    ...
    m_instructions[0x7E] = Instruction(0x7E, 3, "ROR", cycleFunc, workFunc);

    // Normal Jumps
    // 4C nn nn  ------  3   JMP nnnn    Jump Absolute              PC=nnnn
    cycleFunc   = [this] { return 3; }; 
    workFunc    = [this] { setPC(wordOperand()); };
    m_instructions[0x4C] = Instruction(0x4C, 3, "JMP", cycleFunc, workFunc);
    ...
    m_instructions[0x6C] = Instruction(0x6C, 3, "JMP", cycleFunc, workFunc);
    ...
    m_instructions[0x20] = Instruction(0x20, 3, "JSR", cycleFunc, workFunc);
    ...
    m_instructions[0x40] = Instruction(0x40, 1, "RTI", cycleFunc, workFunc);
    // 60        ------  6   RTS         Return from Subroutine     PC=[S]+1
    cycleFunc   = [this] { return 6; }; 
    workFunc    = [this] { setPC(popStackWord() + 1); };
    m_instructions[0x60] = Instruction(0x60, 1, "RTS", cycleFunc, workFunc);
    ...
    m_instructions[0x10] = Instruction(0x10, 2, "BPL", cycleFunc, workFunc);
    // 30 dd     ------  2** BMI disp    Branch on result minus    if N=1 PC=PC+/-nn
    cycleFunc   = [=, this] { return conditionalBranchCycleFunc(m_status.negative(), byteOperand()); }; 
    workFunc    = [this] { conditionalBranch(m_status.negative(), byteOperand()); };
    m_instructions[0x30] = Instruction(0x30, 2, "BMI", cycleFunc, workFunc);
    ...
    m_instructions[0x50] = Instruction(0x50, 2, "BVC", cycleFunc, workFunc);
    // 70 dd     ------  2** BVS disp    Branch on overflow set    if V=1 PC=PC+/-nn
    cycleFunc   = [=, this] { return conditionalBranchCycleFunc(m_status.overflow(), byteOperand()); };
    workFunc    = [this] { conditionalBranch(m_status.overflow(), byteOperand()); };
    m_instructions[0x70] = Instruction(0x70, 2, "BVS", cycleFunc, workFunc);
    ...
    m_instructions[0x90] = Instruction(0x90, 2, "BCC", cycleFunc, workFunc);
    // B0 dd     ------  2** BCS disp    Branch on carry set       if C=1 PC=PC+/-nn
    cycleFunc   = [=, this] { return conditionalBranchCycleFunc(m_status.carry(), byteOperand()); };
    workFunc    = [this] { conditionalBranch(m_status.carry(), byteOperand()); };
    m_instructions[0xB0] = Instruction(0xB0, 2, "BCS", cycleFunc, workFunc);
    ...
    m_instructions[0xD0] = Instruction(0xD0, 2, "BNE", cycleFunc, workFunc);
    // F0 dd     ------  2** BEQ disp    Branch on result zero     if Z=1 PC=PC+/-nn
    cycleFunc   = [=, this] { return conditionalBranchCycleFunc(m_status.zero(), byteOperand()); };
    workFunc    = [this] { conditionalBranch(m_status.zero(), byteOperand()); };
    m_instructions[0xF0] = Instruction(0xF0, 2, "BEQ", cycleFunc, workFunc);
    ...
    m_instructions[0x00] = Instruction(0x00, 1, "BRK", cycleFunc, workFunc);
    ...
    m_instructions[0x18] = Instruction(0x18, 1, "CLC", cycleFunc, workFunc);
    // 58        ---0--  2   CLI         Clear interrupt disable bit I=0
    cycleFunc   = [this] { return 2; };
    workFunc    = [this] { m_status.setIRQDisable(false); };
    m_instructions[0x58] = Instruction(0x58, 1, "CLI", cycleFunc, workFunc);
    // D8        ----0-  2   CLD         Clear decimal mode          D=0
    cycleFunc   = [this] { return 2; };
    workFunc    = [this] { m_status.setDecimalMode(false); };
    m_instructions[0xD8] = Instruction(0xD8, 1, "CLD", cycleFunc, workFunc);
    // B8        -----0  2   CLV         Clear overflow flag         V=0
    cycleFunc   = [this] { return 2; };
    workFunc    = [this] { m_status.setOverflow(false); };
    m_instructions[0xB8] = Instruction(0xB8, 1, "CLV", cycleFunc, workFunc);
    // 38        --1---  2   SEC         Set carry flag              C=1
    cycleFunc   = [this] { return 2; };
    workFunc    = [this] { m_status.setCarry(true); };
    m_instructions[0x38] = Instruction(0x38, 1, "SEC", cycleFunc, workFunc);
    // 78        ---1--  2   SEI         Set interrupt disable bit   I=1
    cycleFunc   = [this] { return 2; };
    workFunc    = [this] { m_status.setIRQDisable(true); };
    m_instructions[0x78] = Instruction(0x78, 1, "SEI", cycleFunc, workFunc);
    // F8        ----1-  2   SED         Set decimal mode            D=1
    cycleFunc   = [this] { return 2; };
    workFunc    = [this] { m_status.setDecimalMode(true); };
    m_instructions[0xF8] = Instruction(0xF8, 1, "STA", cycleFunc, workFunc);

    // No Operation
    // EA        ------  2   NOP         No operation                No operation
    cycleFunc   = [this] { return 2; };
    workFunc    = [this] {};
    m_instructions[0xEA] = Instruction(0xEA, 1, "NOP", cycleFunc, workFunc);
